...
public class StaffScript implements CommandExecutor{

    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        Player p = (Player) sender;

        if (cmd.getName().equalsIgnoreCase("staffinfo")) {
            if(p.hasPermission("staffinfo.help")) {
                if (args.length == 0) {
                    Text text1 = new Text("");
                    Text text2 = new Text("");
                    Text text3 = new Text("");
                ...
                if (args.length == 1) {
                    if (args[0].equalsIgnoreCase("paper")) {
                        ...
                        new Text("[Click to open Xray]").setColor(ChatColor.GREEN).setClick(ClickAction.RUN_COMMAND, "/staffinfo page xray").send(p);
